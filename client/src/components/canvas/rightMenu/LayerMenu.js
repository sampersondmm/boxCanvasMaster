...
import { Menu, Tab, Icon, Accordion, Label } from 'semantic-ui-react';
...
class LayerMenu extends Component {
    ...
    handleOpen = (index) => {
        this.setState(state => {
            const indexIncluded = state.openList.includes(index);
            let { openList } = state;
            if(indexIncluded){
                openList = without(openList, index)
            } else {
                openList.push(index)
            }
            return {
    ...
    render(){
        ...
        const wrapHeight = !inverted ? '433px' : 'calc(100vh - 120px)';
        const scrollClass = !inverted ? 'scrollbar' : 'scrollbar-inverted';
        return (
            <div className={`${scrollClass} tab-pane-wrap`} style={{height: wrapHeight, border: '1px solid rgb(120,120,120)'}}>
                        ...
                        {shapeList.map((shape, index) => {
                            return (
                                <Menu.Item className='shape-accordian-option' style={{margin: '0 2px', marginBottom: '5px', borderRadius: '4px'}}>
                                    ...
                                    <Accordion.Content active={openList.includes(index)} as={Menu} inverted={inverted} style={{margin: '0', marginTop: '5px', border: '0', boxShadow: '0 0 0 0'}}>
